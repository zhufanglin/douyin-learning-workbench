...
type Detail = { business_profile?: {name:string;goal:string;keywords:string[];include:string;exclude:string}; task: HistoryTask; videos: SavedVideo[]; comments: SavedComment[]; users: SavedUser[]; pagination?: object; screenshot_url?: string }
export function LibraryPanel({ view, selected, select }: { view: View; selected: string; select: (id: string) => void }) {
  ...
  const selecting=(view==='tasks'||view==='videos')&&selectionMode===selectionScope
  const [batchPage,setBatchPage]=useState(0)
  const batchVideoSort=useVideoSort(videos.filter(v=>!videoFilter||identity(v)===videoFilter))
  const batchPool = view==='videos' ? batchVideoSort.ordered.map(v=>({target:{kind:'videos' as const,source:v.source,id:v.id},label:v.title||v.id,video:v,user:undefined as SavedUser|undefined,comment:undefined as SavedComment|undefined}))
    : view==='users' ? [...new Map(groupRows.flatMap(g=>g.users).map(u=>[identity(u),u])).values()].map(u=>({target:{kind:'users' as const,source:u.source,id:u.id},label:u.nickname||u.id,video:undefined as SavedVideo|undefined,user:u,comment:undefined as SavedComment|undefined}))
    : comments.map(c=>({target:{kind:'comments' as const,source:c.source,id:c.id},label:c.content||'图片评论',video:undefined as SavedVideo|undefined,user:undefined as SavedUser|undefined,comment:c}))
  const activeBatchPage=Math.min(batchPage,Math.max(0,Math.ceil(batchPool.length/100)-1))
  const batchRows=batchPool.slice(activeBatchPage*100,(activeBatchPage+1)*100)
  const dataSelection=useBatchSelection(batchRows,selectionScope+'|records|'+activeBatchPage,selecting)
  const taskSelection = useBatchSelection(pageSlice(visibleTasks).map(t => ({ target: { kind: 'tasks', source: t.source, id: t.id }, label: taskLabel(t, catalog.data?.videos), disabled: ['running','queued'].includes(t.status) })), selectionScope, selecting)
  ...
  return <div key={selected || 'directory'} className="library-panel learning-detail-enter">
    <div className="flex items-center justify-between gap-3 mb-4"><h2 className="text-xl font-semibold">{selected ? '任务详情' : viewNames[view]}</h2>{!selected && (view==='tasks'||view==='videos') && <Button size="sm" variant="outline" onClick={()=>{setBatchPage(0);setSelectionMode(selecting?'':selectionScope)}}>{selecting?'取消选择':'选择'}</Button>}{selected && <Button size="sm" variant="outline" onClick={() => { select(''); setInitialVideo('') }}>返回{viewNames[view]}</Button>}</div>
    {!selected ? <>
      ...
      <div className="library-filters">
        ...
        <label>来源任务<select className="library-select" aria-label="筛选来源任务" value={taskFilter} onChange={e => { setTaskFilter(e.target.value); setVideoFilter(''); setPage(0) }}><option value="">全部任务</option>{catalog.data?.tasks.map(t => <option key={t.id} value={t.id}>{timeLabel(t.created_at)} · {taskLabel(t, catalog.data?.videos)}</option>)}</select></label>
      ...
      {current.isError ? <div role="alert">数据目录读取失败。<Button size="sm" variant="outline" onClick={() => void current.refetch()}>重试</Button></div> : current.isLoading ? <p className="library-empty">正在加载历史记录…</p> : <>
        ...
        <BatchToolbar selection={view==='tasks'?taskSelection:dataSelection} exportTaskId={taskFilter} metadataTaskId={view==='videos'?tasks.find(t=>dataSelection.chosen.every(r=>t.source===r.target.source&&t.videos.some(v=>v.id===r.target.id)))?.id:''} externalControl />
        {selecting && view!=='tasks' && <div className="unified-selection-list">
          {view==='videos' && batchVideoSort.controls}<p className="text-xs text-cyber-text-secondary my-3">选择范围：当前筛选结果，每页最多100条；跨任务重复记录只显示一次。翻页清空勾选。</p>
          {batchRows.map(row=><article className="batch-video-record" key={identity(row.target)}>
            <RowSelection selection={dataSelection} row={row}/><div className="min-w-0 flex-1">
              {row.video ? <><button className="record-title text-left" title={row.label} onClick={()=>openSavedVideo(row.video!)}>{row.label}</button><VideoStats video={row.video}/></>
              :row.user?<button className="record-title text-left" onClick={()=>setDrawer({user:row.user!})}>{row.label}</button>
              :row.comment?<><strong>{userFor(row.comment).nickname}</strong><CommentContent comment={row.comment}/><small>{timeLabel(row.comment.create_time)}</small></>:row.label}
            </div>
          </article>)}
          <div className="library-pager"><Button size="sm" variant="outline" disabled={!activeBatchPage} onClick={()=>{dataSelection.clear();setBatchPage(activeBatchPage-1)}}>选择上一页</Button><span>第 {activeBatchPage+1}/{Math.max(1,Math.ceil(batchPool.length/100))} 页 · 共 {batchPool.length} 条</span><Button size="sm" variant="outline" disabled={(activeBatchPage+1)*100>=batchPool.length} onClick={()=>{dataSelection.clear();setBatchPage(activeBatchPage+1)}}>选择下一页</Button></div>
        </div>}
        {view === 'tasks' && <div className="history-task-list">{pageSlice(visibleTasks).map(t => <div key={t.id} className="record-with-delete"><RowSelection selection={taskSelection} row={{ target: { kind: 'tasks', source: t.source, id: t.id }, label: taskLabel(t, catalog.data?.videos), disabled: ['running','queued'].includes(t.status) }} /><button className="history-task" onClick={() => openTask(t.id)}><span><strong className="record-title" title={taskLabel(t, catalog.data?.videos)}>{taskLabel(t, catalog.data?.videos)}</strong><small>执行时间：{timeLabel(t.created_at)} · {t.source === 'live' ? '网页读取' : '本地导入'}</small></span><span className="task-counts">视频 {t.counts.videos} · 评论 {t.counts.comments} · 用户 {t.counts.users}<small>{statusLabel[t.status] || t.status} → 查看任务</small></span></button></div>)}</div>}
        {!selecting && view === 'videos' && pageSlice(visibleTasks).map(t => <details key={t.id} className="library-group"><summary><strong className="record-title" title={taskLabel(t, catalog.data?.videos)}>{taskLabel(t, catalog.data?.videos)}</strong><span>{timeLabel(t.created_at)} · {t.counts.videos} 个视频 · {statusLabel[t.status] || t.status}</span></summary><Button variant="ghost" size="sm" onClick={() => openTask(t.id)}>查看任务详情</Button><VideoBatchList selecting={selecting} taskId={t.id} videos={t.videos.filter(v => !videoFilter || identity(v) === videoFilter)} scope={selectionScope + t.id} open={video => openTask(t.id,video.id)} /></details>)}
        {!selecting && (view === 'users' || view === 'comments') && pageSlice(groupRows).map(group => <details className="library-group" key={identity(group.video)}><summary><strong className="record-title" title={group.video.title || group.video.id}>{group.video.title || group.video.id}</strong><span>{group.users.length} 位用户 · {group.comments.length} 条评论</span></summary><div className="p-3"><Button size="sm" variant="outline" onClick={() => openSavedVideo(group.video)}>打开视频评论详情</Button><p className="text-xs text-cyber-text-secondary mt-2">{[...new Set(group.comments.flatMap(c => c.task_ids || []))].map(id => tasks.find(t => t.id === id)).filter(Boolean).map(t => `${timeLabel(t!.created_at)} · ${taskLabel(t!, catalog.data?.videos)}`).join('；') || '见任务详情中的执行时间'}</p></div><GroupContent selecting={selecting} key={view} scope={selectionScope} kind={view} group={group} showUser={user => setDrawer({ user, videoId: group.video.id })} /></details>)}
        {(!selecting || view==='tasks') && total > 20 && <div className="library-pager"><Button size="sm" variant="outline" disabled={!effectivePage} onClick={() => setPage(effectivePage - 1)}>上一页</Button><span>第 {effectivePage + 1} / {Math.ceil(total / 20)} 页</span><Button size="sm" variant="outline" disabled={(effectivePage + 1) * 20 >= total} onClick={() => setPage(effectivePage + 1)}>下一页</Button></div>}
      </>}
      ...
      {detail.data.business_profile && <details className="library-group"><summary>业务目标：{detail.data.business_profile.name}（任务创建时）</summary><div className="p-3 whitespace-pre-wrap"><p>{detail.data.business_profile.goal}</p><p>关键词：{detail.data.business_profile.keywords.join("、")}</p><p>纳入：{detail.data.business_profile.include||"未设置"}</p><p>排除：{detail.data.business_profile.exclude||"未设置"}</p></div></details>}<div className="history-detail-heading"><h3>{taskLabel({...detail.data.task,videos:detail.data.videos},catalog.data?.videos)}</h3><p>执行时间：{timeLabel(detail.data.task.created_at)} · {statusLabel[detail.data.task.status] || detail.data.task.status} · {detail.data.task.source === 'live' ? '网页读取' : '本地导入'}</p>{detail.data.task.search_mode&&<p>搜索方式：{detail.data.task.search_mode==='videos'?'先找视频，按需读取评论':'搜索后，逐个读取评论'}</p>}<p>{detail.data.task.note}</p><a href={'/api/learning/export/' + selected} className="text-cyan-700 underline">导出结果</a>{detail.data.screenshot_url && <a className="text-cyan-700 underline ml-4" href={detail.data.screenshot_url} target="_blank" rel="noreferrer">查看暂停截图</a>}</div>
      ...
      <VideoExplorer key={selected + initialVideo} taskId={selected} videos={detail.data.videos} searching={detail.data.task.status === 'running'} commentTaskId={detail.data.pagination || (detail.data.comments.length && (detail.data.task.source==='import' || (detail.data.videos.length===1 && !detail.data.videos[0].comment_task_id))) ? selected : undefined} initialVideoId={initialVideo} source={detail.data.task.source} />
    </>}
    {drawer && <UserDrawer key={identity(drawer.user)} user={drawer.user} videoId={drawer.videoId} close={() => setDrawer(undefined)} />}
  </div>
}

function GroupContent({ kind, group, showUser, scope, selecting }: { selecting: boolean; scope: string; kind: 'users' | 'comments'; group: { comments: SavedComment[]; users: SavedUser[] }; showUser: (user: SavedUser) => void }) {
